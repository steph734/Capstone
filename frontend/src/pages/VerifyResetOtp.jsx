...
export default function VerifyResetOtp() {
  ...
  useEffect(() => {
    if (!email) {
      const t = setTimeout(() => navigate('/forgot-password', { replace: true }), 2500)
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      navigate('/reset-password', {
        replace: true,
        state: { token: data.token, email: data.email || email },
      })
    } catch (err) {
      setStatus('error')
      setError(
        ...
        <div className="otp-header">
          ...
          <p className="otp-subtitle">
            {email ? (
              ...
              <>We couldn&rsquo;t tell which email to verify. Taking you back&hellip;</>
            )}
          </p>
        </div>
