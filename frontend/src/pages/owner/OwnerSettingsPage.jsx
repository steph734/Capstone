...
const SETTINGS_KEY = 'therapypro_owner_settings'
...
const DEFAULT_SETTINGS = {
  ...
  privacy: {
    ...
    shareAnalyticsWithManagers: true,
  },
  security: {
    loginAlerts: true,
  },
...
export default function OwnerSettingsPage({ user, onLogout, betaTier }) {
  ...
  return (
    <OwnerPageShell
      ...
      onLogout={onLogout}
      title="Settings"
      subtitle="Manage your account, notifications, and practice preferences"
      icon="⚙️"
      menuItems={getOwnerMenuItems(betaTier)}
    >
      <div className="settings-container">
        <SettingsSection icon={<UserIcon />} title="My Account" description="Your name, photo, and owner profile">
          <div className="settings-row">
            <div className="settings-row-text">
              <span className="settings-row-label">{user?.name || 'Owner Profile'}</span>
              <span className="settings-row-desc">Update your photo, name, and contact details</span>
            </div>
            <button className="settings-link-btn" onClick={() => navigate('/owner/profile')}>View Profile</button>
          </div>
        </SettingsSection>

        <SettingsSection icon={<BellIcon />} title="Notifications" description="Choose which practice activity you want to be alerted about">
          <ToggleRow
            label="New appointment bookings"
            description="When a patient books or reschedules a session"
            checked={settings.notifications.newBookings}
            onChange={(v) => updateNotification('newBookings', v)}
          />
          <ToggleRow
            label="Payments received"
            description="When a session payment is recorded"
            checked={settings.notifications.paymentsReceived}
            onChange={(v) => updateNotification('paymentsReceived', v)}
          />
          <ToggleRow
            label="Staff attendance alerts"
            description="When a therapist is marked late or absent"
            checked={settings.notifications.staffAttendance}
            onChange={(v) => updateNotification('staffAttendance', v)}
          />
          <ToggleRow
            label="Billing & subscription reminders"
            description="Upcoming invoices and plan renewal notices"
            checked={settings.notifications.billingReminders}
            onChange={(v) => updateNotification('billingReminders', v)}
          />
        </SettingsSection>

        <SettingsSection icon={<EyeIcon />} title="Privacy" description="Control who can see your information across the practice">
          <ToggleRow
            label="Show my profile to staff"
            description="Therapists and admins can view your basic owner info"
            checked={settings.privacy.showProfileToStaff}
            onChange={(v) => updatePrivacy('showProfileToStaff', v)}
          />
          <ToggleRow
            label="Share analytics with branch managers"
            description="Let branch managers see the Reports dashboards"
            checked={settings.privacy.shareAnalyticsWithManagers}
            onChange={(v) => updatePrivacy('shareAnalyticsWithManagers', v)}
          />
        </SettingsSection>
